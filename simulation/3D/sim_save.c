...
int main() {
    ...
    int frame_number = 0;
    char* frame_title = (char*)malloc(max_size*sizeof(char));
    ...
    Camera camera = { 0 };
    camera.position = (Vector3){ 2.5*ARRAY_WIDTH, ARRAY_HEIGHT/2, 2.5*ARRAY_WIDTH };
    ...
    for (int i = 0; i < SIM_N_Y; i++) {
        for (int j = 0; j < SIM_N_XZ; j++) {
            for (int k = 0; k < SIM_N_XZ; k++) {
                ...
                // GET FOCUS, CALCULATE PHI, CALCULATE POWER
                Vector3 focus = sim_pts[i][j][k];
                ...
                // DRAW SIM POINTS
                for (int x = 0; x < SIM_N_Y; x++) {
                    for (int y = 0; y < SIM_N_XZ; y++) {
                        for (int z = 0; z < SIM_N_XZ; z++) {
                            Vector3 pt = sim_pts[x][y][z];
                            ...
                            float p = power_pts[x][y][z];
                            if (p > 50) {
                                Color color = {p, p, p, 0x80}; // ColorFromHSV(p, 100.0, 100.0);
                                DrawSphere(pt, 2.5f, color);
                            }
                        }
                    }
                }
                DrawSphere(focus, 2.0f, RED);
                EndMode3D();
                EndDrawing();

                snprintf(frame_title, max_size, "frames/frame_%d.png", frame_number);
                TakeScreenshot(frame_title);  
                frame_number++;
